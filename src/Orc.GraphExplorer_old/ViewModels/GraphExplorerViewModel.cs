namespace Orc.GraphExplorer.ViewModels
{
	...
	public class GraphExplorerViewModel : ViewModelBase
	{
		...
		public GraphExplorerViewModel()
		{
			...
			Editor = new Editor(serviceLocator.ResolveType<IEditorService>());
		...
		{
			GraphDataService.GetEdges(OnEdgeLoaded, Explorer.OperationObserver.OnError);
		}

		private void OnEdgeLoaded(IEnumerable<DataEdge> edges)
		{
			Edges = edges;
            GraphDataService.GetVertexes(OnVertexesLoaded, Explorer.OperationObserver.OnError);
		}
		...
		{
			foreach (var vertex in Editor.Logic.Graph.Vertices) {
		...
		private void vertex_IsExpandedChanged(object sender, EventArgs e)
		{
            ...
            if (vertex.IsExpanded || vertex.Properties == null || vertex.Properties.Count < 1 || !Editor.Logic.Graph.ContainsVertex(vertex))
            {
				return;
			}
		...
		{
			try {
                GraphDataService.UpdateEdges(Editor.Logic.Graph.Edges, (result, error) =>
                {
                ...
                GraphDataService.UpdateVertexes(Editor.Logic.Graph.Vertices, (result, error) =>
                {
					if (!result && error != null) {
						ShowAlertMessage(error.Message);
					}
        ...
        // TODO: Move it to the Navigator service
		public void ClearEdgeDrawing()
		{
			if (EdFakeDV != null) {
                Editor.Logic.Graph.RemoveVertex(EdFakeDV);
			}
			if (Editor.Service.IsEdgeEditing) {
				var edge = Editor.Service.GetEdEdge();
				// TODO: refactor this
				if (edge != null) {
                    Editor.Logic.Graph.RemoveEdge(edge);
				}
			}
			EdGeometry = null;
			EdFakeDV = null;
		...
		{
            Explorer.OperationObserver.Undo(Editor);
		}
		...
		{
            Explorer.OperationObserver.Redo(Editor);
		}
        ...
        {
            get { return GetValue<GraphExplorer>(ExplorerProperty); }
        ...
        /// </summary>
        public static readonly PropertyData ExplorerProperty = RegisterProperty("Explorer", typeof(GraphExplorer));
        ...
        {
            get { return GetValue<Navigator>(NavigatorProperty); }
            private set { SetValue(NavigatorProperty, value); }
        }

        /// <summary>
        /// Register the Navigator property so it is known in the class.
        /// </summary>
        public static readonly PropertyData NavigatorProperty = RegisterProperty("Navigator", typeof(Navigator));
		...
		public Editor Editor {
			get { return GetValue<Editor>(OperationObserverProperty); }
		...
		/// Register the Navigator property so it is known in the class.
		/// </summary>
        public static readonly PropertyData OperationObserverProperty = RegisterProperty("Editor", typeof(Editor), null);
		...
		protected override void Initialize()
		{
            ...
            ApplySetting(Editor.Logic);
			ApplySetting(Navigator.Logic, true);
		...
		{
            var degree = Editor.Logic.Graph.Degree(dataVertex);
			...
			//overrallGraph.get
            NavigateHistoryItem historyItem = GetHistoryItem(dataVertex, Editor.Logic.Graph);
		...
		{
			SelectedVertices.Clear();
            Explorer.OperationObserver.Clear(Editor);
		...
		public void CreateEdge(int fromId, int toId)
		{
			...
			GraphArea area = View.Area;
            DataVertex source = Editor.Logic.Graph.Vertices.FirstOrDefault(x => x.ID == fromId);
            DataVertex target = Editor.Logic.Graph.Vertices.FirstOrDefault(x => x.ID == toId);
			if (source == null || target == null) {
				return;
			}

            Explorer.OperationObserver.Do(new CreateEdgeOperation(Editor, area, source, target, e =>
            {
				//on vertex created
				//SelectedVertices.Add(v.ID);
                var edgeView = (EdgeView)e;
                if (edgeView.ViewModel == null)
                {
                    edgeView.ViewModelChanged += (sender, args) => edgeView.ViewModel.Data.IsHighlightEnabled = false;
                }
                else
                {
                    edgeView.ViewModel.Data.IsHighlightEnabled = false;
                }
		...
		public void OnVertexLoaded(IEnumerable<DataVertex> vertexes, bool clearHistory = false)
		{
			...
			foreach (DataVertex vertex in vertexes) {
                _observers.Add(vertex.Subscribe(Explorer.OperationObserver));
		...
		private void vertex_OnPositionChanged(object sender, VertexPositionChangedEventArgs e)
		{
			...
			var vertex = (DataVertex)sender;
            if (Editor.Logic.Graph.Vertices.Any(v => v.ID == vertex.ID))
            {
				VertexControl vc = View.Area.VertexList.First(v => v.Key.ID == vertex.ID).Value;
				//throw new NotImplementedException();

                Explorer.OperationObserver.OnNext(new VertexPositionChangeOperation(Editor, View.Area, (GraphLogic)View.Area.LogicCore, vc, e.OffsetX, e.OffsetY, vertex));
			}
		}
		#endregion
